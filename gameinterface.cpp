...
GameInterface::GameInterface(QWidget *parent, WindowManager* manager) :
  QDialog(parent),
  ui(new Ui::GameInterface)
{
  ...
  m_manager=manager;
  Controller=nullptr;
  QObject::connect(manager,SIGNAL(startGame()),this,SLOT(loadGame()));
}

//切换回游玩地图选择界面
void GameInterface::switchback(){
  QM->stop(); //停止BGM
  ...
  m_manager->switchTo(3);//切换窗口到游玩地图选择界面
}

//帧数计时器到时对应的槽
void GameInterface::timeout(){
...
void GameInterface::paintEvent(QPaintEvent* ev){
...
void GameInterface::win(){
  double Score=Controller->GameScore+10000-Timer->elapsed()/10; //计算最终分数
  EditLeaderBoardDialog* Dialog=new EditLeaderBoardDialog(Score,this);
  //保存获得的积分
  if (Dialog->exec()==QDialog::Accepted){
    ...
    QString output("");
    out << QString::number(Score,'f',2) << Qt::endl;
    out << Dialog->getName() << Qt::endl;
    out << QDateTime::currentDateTime().toString("yyyy.MM.dd hh:mm") << Qt::endl;
    file->close();
  }
}
